...
create table edureka_dw.new_employees_in_source stored as orc as 
SELECT 
    src.*,
...
FROM edureka_dw.employees_delta src
left outer join 
...
-- add updated records from source with current date as start date and 9999-12-31 as end date
...
-- get unchanged records from scd which are not present in ch   anged employee codes list
